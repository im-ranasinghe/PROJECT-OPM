...
<style>
    .reportview-container {
        background-color: rgb(195, 62, 218);
    }
    .status-card {
        padding: 1.5rem;
        border-radius: 1rem;
  ...
  const onResults = (results: any) => {
    ...
    if (results.poseLandmarks) {
      ...
      if (isUpperBodyVisible) {
        ...
        const drawColor = postureState === 'warning' ? '#f43f5e' : 'rgb(14, 223, 241)';
        const fillDotColor = postureState === 'warning' ? '#e11d48' :'rgb(235, 235, 237)';
  ...
  return (
    <div id="office_posture_monitor_app" className="min-h-screen bg-slate-950 flex flex-col relative text-slate-100">
              ...
              <div className="relative flex-1 min-h-[400px] md:min-h-[500px] rounded-2xl overflow-hidden bg-slate-900 border border-slate-800/80 shadow-2xl flex flex-col items-center justify-center">
                ...
                {cameraState === 'active' && (
                  <>
                    ...
                    <div className="absolute top-4 right-4 px-3 py-1.5 rounded-xl bg-gradient-to-br from-sky-50 via-white to-indigo-100/80 backdrop-blur-md border border-slate-800/80 shadow-md text-xs font-mono flex items-center gap-2 select-none pointer-events-none text-slate-300">
      ...
      <footer className="py-4 bg-slate-950 text-center border-t border-slate-900 mt-auto text-[11px] text-slate-600 font-mono select-none">
        Office Posture Monitor &copy; 2026 im.ranasinghe
      </footer>
    </div>
  );
}
